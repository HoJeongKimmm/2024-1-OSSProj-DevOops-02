...
export const ProjectCardDongguk: FC<ProjectCardProps> = ({ className, projectItem, isInvited, status }) => {
  const navigate = useNavigate()

  const onClickRoot = () => {
    if (isInvited) {
      navigate(`/projectDongguk/invite/${projectItem.key}`)
    } else {
      navigate(`/projectDongguk/${projectItem.key}`)
    }
  }
